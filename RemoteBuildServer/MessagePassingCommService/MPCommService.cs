 ...
 * --------------------
 *      ver 3.0 : Nov 01, 2017
 *      - changed Sender's PostFile(), it now takes arguments
 *      - before sending a file, checks if last url is used and sends it,
 *          or else, creates a new Send channel and then sends the files
 *          
 *      ver 2.1 : 20 Oct 2017
...
namespace MessagePassingComm
{
    ...
    public class Sender
    {
        ...
        public bool postFile(string fileName, string fromPath, string toPath, string toEndPoint)
        {
            FileStream fs = null;
            long bytesRemaining;
    ...
    public class Comm
    {
        private Receiver rcvr { get; set; } = null;
        private Sender sndr { get; set; } = null;
        ...
        public bool postFile(string filename, string fromPath, string toPath, string toEndPoint)
        {
            return sndr.postFile(filename, fromPath, toPath, toEndPoint);
        }
        public void close()
        {
            Console.Write("\n Comm is closing");
    ...
    class TestPCommService
    {
        ...
        //#if (TEST_COMM_SERVICE)
        static void Main(string[] args)
        {
            //TestPCommService commServiceObj = new TestPCommService();

            Console.WriteLine("\n Calling testSndrRcvr()");
            //TestPCommService.testSndrRcvr();
            Console.WriteLine("\n which creates a Sender and a Receiver object and fulfills the rest of the functionality");
        }
        //#endif
    }
}
